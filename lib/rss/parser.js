...
export async function parseFeed(feedUrl, timeout = 8000) {
  const { articles } = await fetchAndParseFeed(feedUrl, { timeout });
  return articles;
}

/**
 * Fetch + parse a feed with conditional-GET (ETag / Last-Modified) support.
 *
 * When etag/lastModified from the previous fetch are supplied, the server may
 * respond 304 Not Modified — in which case we skip parsing entirely. This cuts
 * bandwidth and CPU dramatically for feeds that rarely change.
 *
 * @param {string} feedUrl
 * @param {{ timeout?: number, etag?: string|null, lastModified?: string|null }} opts
 * @returns {Promise<{ articles: Array, notModified: boolean, etag: string|null, lastModified: string|null, status: number }>}
 */
export async function fetchAndParseFeed(feedUrl, { timeout = 8000, etag = null, lastModified = null } = {}) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeout);

  try {
    const headers = {
      'User-Agent': 'FeedPrism/1.0 (+https://feedprism.app) RSS Reader',
      'Accept': 'application/rss+xml, application/atom+xml, application/xml, text/xml',
    };
    if (etag) headers['If-None-Match'] = etag;
    if (lastModified) headers['If-Modified-Since'] = lastModified;

    const response = await fetch(feedUrl, { signal: controller.signal, headers });

    if (response.status === 304) {
      return { articles: [], notModified: true, etag, lastModified, status: 304 };
    }
    ...
    if (parsed.rss?.channel) {
      articles = parseRSSItems(parsed.rss.channel);
    } else if (parsed.feed?.entry) {
      articles = parseAtomEntries(parsed.feed);
    } else {
      const channel = parsed?.rss?.channel || parsed?.channel || parsed?.feed;
      if (channel) articles = parseRSSItems(channel);
    }
